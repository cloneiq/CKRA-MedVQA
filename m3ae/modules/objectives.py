...
def compute_vqa(pl_module, batch, test=False):
    ...
    loss_corr = infer["loss_corr"]
    loss_ctra = infer["loss_ctra"]
    vqa_loss = (F.binary_cross_entropy_with_logits(vqa_logits, vqa_targets) * vqa_targets.shape[1]) + 0.01 * loss_corr + 2.0 * loss_ctra
    logits = vqa_logits.detach().cpu().numpy()
    max_indices = np.argmax(logits, axis=1)
    preds = np.zeros_like(logits)
    for i, idx in enumerate(max_indices):
        preds[i, idx] = 1
    targets = vqa_targets.detach().cpu().numpy()
    ...
    pl_module.log(f"vqa/{phase}/score", f1)
