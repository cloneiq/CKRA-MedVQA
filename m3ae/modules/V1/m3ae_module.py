...
class M3AETransformerSS(pl.LightningModule):
    ...
    def infer(
        ...
        extended_image_masks = self.language_encoder.get_extended_attention_mask(image_masks, image_masks.size(),
                                                                                 device)
#         print("text@@@@", uni_modal_text_feats.shape)
#         print("text mask@@@", extended_text_masks)
#         print("image@@@@", uni_modal_image_feats.shape)
#         print("image mask@@@", extended_image_masks.shape)
#         sys.exit()
        # == End  : Image Encoding ==
...
#         shape_k = k.shape[1]
#         shape_xm = list(extended_text_masks.shape)
#         shape_xm[-1] = shape_k
#         extended_knowledge_masks = torch.ones(shape_xm).to(device)
#         print(extended_knowledge_masks.shape)
#         sys.exit()
#         print(x.shape, y.shape, k.shape)   # torch.Size([4, 32, 768]) torch.Size([4, 577, 768]) torch.Size([4, 32, 768])
#         print(y_f.shape, k_f.shape)   # torch.Size([4, 32, 768]) torch.Size([4, 577, 768]) torch.Size([4, 32, 768])
#         print(extended_text_masks.shape, extended_image_masks.shape)  # torch.Size([4, 1, 1, 32]) torch.Size([4, 1, 1, 577])
#         sys.exit()
...
#             print(y.shape, k.shape)  # torch.Size([8, 577, 768]) torch.Size([8, 196, 768])
#             sys.exit()
            # == End: Co-Attention ==
